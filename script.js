...
function toggleStartMenu() {
    ...
    if (startMenu.classList.contains("open")) {
        startSearch.value = "";
        filterStartApps();
        window.setTimeout(() => startSearch.focus(), 50);
    }
}
...
function saveInstalledApps() {
    try {
        localStorage.setItem("novaInstalledApps", JSON.stringify([...installedApps]));
    } catch {
...
function syncInstalledApps() {
    document.querySelectorAll("[data-install-app]").forEach((button) => {
        ...
        button.textContent = isInstalled ? "Open" : "Install";
        button.classList.toggle("installed", isInstalled);
    });

    document.querySelectorAll("[data-installed-app]").forEach((button) => {
        button.hidden = !installedApps.has(button.dataset.installedApp);
    });
...
document.querySelectorAll("[data-install-app]").forEach((button) => {
    button.addEventListener("click", () => {
        installOrOpenApp(button.dataset.installApp);
    });
});
...
function filterStartApps() {
    ...
    document.querySelectorAll(".start-app").forEach((button) => {
        const requiredInstall = button.dataset.installedApp;
        const isAvailable = !requiredInstall || installedApps.has(requiredInstall);
        const matches = button.dataset.startName.includes(query);
        const shouldShow = isAvailable && matches;

        button.hidden = !shouldShow;
        if (shouldShow) visibleApps += 1;
    });

    document.getElementById("start-empty").hidden = visibleApps !== 0;
}
...
storeSearch.addEventListener("input", () => {
    ...
    document.getElementById("store-empty").hidden = visibleApps !== 0;
});

try {
    const savedApps = JSON.parse(localStorage.getItem("novaInstalledApps") || "[]");
    installedApps = new Set(Array.isArray(savedApps) ? savedApps : []);
} catch {
...
try {
    notesTextarea.value = localStorage.getItem("novaNotes") || "";
} catch {
    notesTextarea.value = "";
}
...
notesTextarea.addEventListener("input", () => {
    ...
    notesSaveTimer = window.setTimeout(() => {
        try {
            localStorage.setItem("novaNotes", notesTextarea.value);
...
document.querySelectorAll("[data-calculator-value]").forEach((button) => {
    button.addEventListener("click", () => {
        if (calculatorDisplay.value === "Error") {
            calculatorExpression = "";
        }

        calculatorExpression += button.dataset.calculatorValue;
        updateCalculatorDisplay(calculatorExpression);
    });
});

document.querySelectorAll("[data-calculator-action]").forEach((button) => {
    button.addEventListener("click", () => {
        ...
        if (action === "equals") {
            const isSafeExpression = /^[0-9+\-*/.() ]+$/.test(calculatorExpression);

            if (!isSafeExpression || !calculatorExpression) {
...
document.querySelectorAll(".menu-dropdown [data-action]").forEach((button) => {
    button.addEventListener("click", async () => {
        const action = button.dataset.action;
        closeTopMenus();

        if (action === "open-files") openApp("files");
        if (action === "open-browser") openApp("browser");
        if (action === "open-settings") openApp("settings");
        if (action === "open-store") openApp("store");
        if (action === "close-active") closeActiveWindow();
        if (action === "next-wallpaper") changeWallpaper((currentWallpaperIndex + 1) % wallpapers.length);
        ...
        if (action === "select-all") {
            ...
            if (activeField instanceof HTMLInputElement) {
                ...
                showNovaPrompt("Edit", "Click inside a text field first, then choose Select All.");
            }
        }
        ...
        if (action === "clear-field") {
            ...
            if (activeField instanceof HTMLInputElement) {
                ...
                showNovaPrompt("Edit", "Click inside a text field first, then choose Clear Field.");
            }
        }
        ...
        if (action === "fullscreen") {
            try {
                ...
                showNovaPrompt("Full Screen", "Your browser did not allow NovaOS to enter full screen.");
            }
        }

        if (action === "shortcuts") {
            showNovaPrompt("Keyboard Shortcuts", "Esc — close the active window\nCtrl+A — select text\nEnter — submit login or browser search");
        }

        if (action === "about") {
            showNovaPrompt("About NovaOS", "NovaOS is a browser-based desktop experience built with HTML, CSS, and JavaScript.");
        }
    });
});
...
document.addEventListener("keydown", (event) => {
    ...
    const openMenu = document.querySelector(".top-menu.open");
